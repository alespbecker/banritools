        ...
        {navItems.filter((i) => !i.adminOnly || isAdminLike).map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 rounded-md h-9 pl-4 pr-3 text-sm font-medium transition-colors",
                isActive
      ...
      <div className="border-t border-border/60 p-2 space-y-1">
        {onToggleTheme && (
          <button
            ...
            className="flex w-full items-center gap-3 rounded-md h-9 pl-4 pr-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
          >
